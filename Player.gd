...
func onSelectingTile(tilePosition: Vector2i, allOccupiedTiles: Array[Vector2i], enemyOccupiedTiles: Array[Vector2i]):
	...
	if state == STATES.WAITING_FOR_MOVE_SELECTION:
	...
	for piece in pieces:
		if piece.move.localPosition == tilePosition:
			...
			if piece.health.isDead():
				pieceDied.emit(attackerPiece, piece, tilePosition)
				pieces.erase(piece)
				piece.queue_free()
			else:
...
func startTurn(nextPiece: Piece) -> void:
	currentlySelectedPiece = nextPiece
	pieceSelected.emit(nextPiece)
	state = STATES.WAITING_FOR_MOVE_SELECTION
